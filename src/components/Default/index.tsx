...
export function HomePage({ id }: { id: string | null }) {
  ...
  return (
    <main className=" overflow-hidden">
      {loading ? <Loading /> : (<><NavbarDemo className="top-0 fixed" data={profileData} />
        <div className="w-full bg-black flex-col md:flex-row pb-20  justify-center items-center">
          <Hero />
          {id && paragraphs && <h1 className="text-neutral-400 text-5xl text-center font-bold">{paragraphs?.title}</h1>}
          ...
          {id && paragraphs && <TextGenerate title={"Alignment with requirements"} text={paragraphs?.body} />}
          <CardHoverEffectDemo data={settingsData["Skills"]} />
          {id && paragraphs && <TextGenerate title={"My closing statement (powered by gpt)"} text={paragraphs?.conclusion} />}
